...
public final class ControlInputHandler {
    ...
    private static boolean prevSpace;
    private static boolean prevV;
    ...
    public static void start(BlockPos pos) { consolePos = pos; prevSpace = false; prevV = false; exiting = false; }
    ...
    private static void tick(ClientTickEvent.Post e) {
        ...
        if (shift && !exiting) {
            exiting = true;
            PacketDistributor.sendToServer(new ControlInputPacket(consolePos, false, false, true));
            return;
        }

        boolean changed = space != prevSpace || v != prevV;
        if (changed) {
            PacketDistributor.sendToServer(new ControlInputPacket(consolePos, space, v, false));
            prevSpace = space;
            prevV = v;
        }
    }
